...
function envothemes_pro_notice() {
    ...
    if (time() - $activation_time > $daysinseconds) {
        if (defined('ENVO_ECOMMERCE_PRO_CURRENT_VERSION') || defined('ENVO_STOREFRONT_PRO_CURRENT_VERSION')) {
            return;
        }
...
function envothemes_shop_notice() {
    ...
    envothemes_shop_dismiss();

    $activation_time = get_site_option('envothemes_active_shop_time');

    if (!$activation_time) {
        add_site_option('envothemes_active_shop_time', time());
    }
    ...
    if (time() - $activation_time > $daysinseconds) {
        ...
        add_action('admin_notices', 'envothemes_shop_notice_message');
    }
}
 ...
 * For shop notice 
 */
function envothemes_shop_notice_message() {
    $scheme = (parse_url($_SERVER['REQUEST_URI'], PHP_URL_QUERY)) ? '&' : '?';
    $url = $_SERVER['REQUEST_URI'] . $scheme . 'envothemes_shop_dismiss=yes';
    $dismiss_url = wp_nonce_url($url, 'envo-shop-nonce');
    $theme = wp_get_theme();
    $themetemplate = $theme->template;
    $themename = $theme->name;
    ?>
    ...
    <div class="envo-review-notice envo-shop-notice">
        <div class="envo-review-thumbnail">
            <img src="<?php echo esc_url(ENVO_URL) . 'img/envo-shop-logo.png'; ?>" alt="">
        </div>
        <div class="envo-review-text">
            ...
            <p>
                ...
                echo sprintf(
                        ...
                        '<a href="https://envothemes.com/free-envo-shop/" target="_blank">Envo Shop</a>',
                        '<a href="https://envothemes.com/envo-shop/" target="_blank">Demo</a>')
                ?>
            </p>
            <ul class="envo-review-ul">
                <li class="show-mor-message">
                    <a href="https://envothemes.com/free-envo-shop/" target="_blank">
                        <span class="dashicons dashicons-external"></span>
                        <?php esc_html_e('Show me more', 'envothemes-demo-import') ?>
                    </a>
                </li>
 ...
 * For shop Dismiss! 
 */
function envothemes_shop_dismiss() {

    if (!is_admin() ||
            !current_user_can('manage_options') ||
            !isset($_GET['_wpnonce']) ||
            !wp_verify_nonce(sanitize_key(wp_unslash($_GET['_wpnonce'])), 'envo-shop-nonce') ||
            !isset($_GET['envothemes_shop_dismiss'])) {

        return;
    }
    $daysinseconds = 1009600; // 14 Days in seconds (1209600).
    $newtime = time() + $daysinseconds;
    update_site_option('envothemes_active_shop_time', $newtime);
}
